...
struct MyStruct
{
    ...
    void PrintTest(void) const  // Without this const, we can't access any of the members/ member function, because lambda args are passed as const
    {
        std::cout << "PrintTest\n";
    }
};
...
int main()
{
    ...
    // capture by value - All
    {
        MyStruct ms2;
        std::cout << "Creating Lambda having MyStruct as argument\n";
        auto f1 = [=](){}; // everything captured as value, since none of the variable used inside lambda, nothing passed, resulting no MyStruct constructor/ destrucor call
        std::cout << "Destroying Lambda having MyStruct as argument\n";
    }
    std::cout << "\n";
    // capture by value - All
